...
${JSON.stringify(relevantContext, null, 2)}`;
      ...
      try {
        ...
        console.log('[SERVER] Gemini response successfully received via gemini-3.7-flash');
        ...
        try {
          ...
          console.log('[SERVER] Gemini response successfully received via gemini-flash-latest');
      ...
      if (!responseText.trim()) {
        console.error('[SERVER] Gemini API Technical Error: Empty response from model.');
        return res.status(503).json({
          error: 'PlaceFlow AI is temporarily unavailable. Please try again.',
          response: 'PlaceFlow AI is temporarily unavailable. Please try again.'
        });
