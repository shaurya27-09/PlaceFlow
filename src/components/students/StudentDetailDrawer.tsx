...
export const StudentDetailDrawer: React.FC<StudentDetailDrawerProps> = ({ student, onClose, onEdit }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-slate-900/60 backdrop-blur-xs flex justify-end" onClick={onClose}>
      <div 
        className="w-full sm:max-w-xl md:max-w-2xl bg-white dark:bg-slate-900 h-full shadow-2xl overflow-y-auto border-l border-slate-200 dark:border-slate-800 p-4 sm:p-6 flex flex-col justify-between animate-in slide-in-from-right duration-300"
      ...
      >
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between pb-4 border-b border-slate-100 dark:border-slate-800 gap-2">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center font-bold text-lg shadow-sm shrink-0">
                {student?.name?.charAt(0) || 'S'}
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white truncate">{student?.name || 'Student'}</h2>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">
                  Roll: <span className="font-mono font-semibold text-slate-700 dark:text-slate-300">{student?.enrollmentNumber || '—'}</span> • {student?.branch || '—'} • {student?.graduationYear || 2026}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {onEdit && (
                <button
                  ...
                  className="p-2 sm:p-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors flex items-center gap-1 text-xs font-semibold shadow-2xs min-h-[36px] touch-manipulation"
                  title="Edit Student"
                >
              ...
              <button
                ...
                className="p-2 sm:p-1.5 rounded-lg border border-rose-200 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 hover:bg-rose-100 transition-colors flex items-center gap-1 text-xs font-semibold shadow-2xs min-h-[36px] touch-manipulation"
              ...
              <button
                ...
                onClick={onClose}
                className="p-2 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 min-h-[36px] min-w-[36px] flex items-center justify-center touch-manipulation"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
                    ...
                    {offer.status === 'Pending' && (
                      <div className="flex items-center gap-1.5 shrink-0">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition-colors min-h-[36px] touch-manipulation"
                        >
                          Accept
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 text-slate-700 dark:text-slate-300 font-semibold text-xs transition-colors min-h-[36px] touch-manipulation"
                        >
                          Decline
                        </button>
                      </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 space-y-3">
            ...
            <div className="text-xs">
              ...
              <select
                ...
                className="w-full px-3 py-2.5 text-base sm:text-xs rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-white min-h-[40px] touch-manipulation"
              >
                {(drives || []).map(d => (
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end text-xs">
          <button
            onClick={onClose}
            className="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-200 min-h-[40px] touch-manipulation cursor-pointer"
          >
            Close Profile
          </button>
        </div>
