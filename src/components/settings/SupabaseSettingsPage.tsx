...
export const SupabaseSettingsPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'status' | 'liveOffers' | 'sqlSchema'>('status');
  ...
  const handleTestDirectly = async () => {
    ...
    if (result.success) {
      addToast('Connection Verified', 'Successfully verified database connection.', 'success');
      loadLiveOffers();
    } else {
      addToast('Connection Notice', result.message, 'warning');
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            {isSupabaseConfigured && supabaseConnected ? (
              ...
              <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-blue-100 dark:bg-blue-950/70 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800 flex items-center gap-1.5">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Read-only database diagnostics and live synchronization state for PlaceFlow.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isTesting}
            className="px-3.5 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/60 text-slate-700 dark:text-slate-200 text-xs font-bold transition-colors flex items-center gap-1.5 disabled:opacity-50 cursor-pointer"
          >
            ...
            <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs space-y-5">
              ...
              <div className="space-y-3">
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">Connection Status</span>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Active link with Supabase PostgreSQL</p>
                  </div>
                  {isSupabaseConfigured && supabaseConnected ? (
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 flex items-center gap-1.5 border border-emerald-200 dark:border-emerald-800">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                      Connected
                    </span>
                  ) : isSupabaseConfigured ? (
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 dark:bg-blue-950/80 text-blue-700 dark:text-blue-300 flex items-center gap-1.5 border border-blue-200 dark:border-blue-800">
                      <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />
                      Configured
                    </span>
                  ) : (
                    <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                ...
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">Configuration Authority</span>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Where credentials originate</p>
                  </div>
                ...
                <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">Client-Side Overrides</span>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">Browser/localStorage credential overrides</p>
                  </div>
                  <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                    Disabled (Security Enforced)
                  </span>
                </div>
              </div>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-center gap-3">
                <button
                  ...
                  disabled={isSyncing}
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold shadow-md shadow-emerald-600/25 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                ...
                <button
                  ...
                  disabled={isSyncing}
                  className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/60 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
          ...
          <div className="space-y-4">
            <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-indigo-950 text-white shadow-xl space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h4 className="font-bold text-xs">Security Hardened</h4>
              </div>
              ...
              <div className="space-y-2.5 text-[11px] text-slate-300">
                <div className="flex items-start gap-2">
                  ...
                  <p>Database credentials are provisioned exclusively via system environment variables and are never stored in browser storage.</p>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <p>Client-side credential modification is disabled. All database operations respect Row Level Security (RLS) policies.</p>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <p>The Eligibility Engine and placement operations use the authorized Supabase client securely.</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
            >
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer"
            >
